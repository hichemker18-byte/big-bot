...
class DatabaseManager:
    ...
    def adapt_query(query: str) -> str:
        ...
        q = query.replace('%s', '?')
        q = re.sub(r'EXTRACT\s*\(\s*EPOCH\s+FROM\s+NOW\(\)\s*\)\s*::\s*BIGINT', "strftime('%s', 'now')", q, flags=re.IGNORECASE)
    ...
    def init_db(self):
        ...
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
            exec_sql = lambda sql, p=(): cursor.execute(self.adapt_query(sql) if self.is_sqlite else sql, p)
            ...
            ''')
            exec_sql('CREATE INDEX IF NOT EXISTS idx_user_id ON admins(user_id)')
            exec_sql('CREATE INDEX IF NOT EXISTS idx_username ON admins(username)')
            ...
            ''')
            exec_sql('CREATE INDEX IF NOT EXISTS idx_sent_batch ON sent_messages(batch_id)')
            ...
            ''')
            exec_sql('CREATE INDEX IF NOT EXISTS idx_listing_user ON listing_requests(user_id)')
            ...
            ''')
            exec_sql('CREATE INDEX IF NOT EXISTS idx_blacklist_target ON blacklist(target_id)')
            ...
            ''')
            exec_sql('CREATE INDEX IF NOT EXISTS idx_sched_del ON scheduled_deletions(status, delete_at)')
            ...
            ''')
            exec_sql('CREATE INDEX IF NOT EXISTS idx_channel_strikes ON channel_strikes(channel_id)')

            exec_sql('SELECT user_id FROM admins WHERE user_id = %s', (ADMIN_ID,))
            if not cursor.fetchone():
                exec_sql(
                    'INSERT INTO admins (user_id, username) VALUES (%s, %s)',
                    (ADMIN_ID, 'المشرف الأساسي')
                )

            conn.commit()
            db_mode = "SQLite محلياً" if self.is_sqlite else "PostgreSQL سحابياً"
            logger.info(f"✅ تم تهيئة قاعدة البيانات بنجاح ({db_mode})")
        except Exception as e:
