...
class DatabaseManager:
    ...
    def __init__(self):
        db_url = os.environ.get('DATABASE_URL') or os.environ.get('SUPABASE_DB_URL') or os.environ.get('POSTGRES_URL') or DATABASE_URL
        ...
        if self.database_url and HAS_PSYCOPG2:
            try:
                conn_test = psycopg2.connect(self.database_url)
                conn_test.close()
                self.is_sqlite = False
                logger.info("🐘 تم تفعيل والاتصال بنجاح بقاعدة البيانات السحابية PostgreSQL (Supabase / Render).")
            except Exception as pg_err:
                logger.error(f"⚠️ فشل الاتصال برابط PostgreSQL ({pg_err})! سيتم استخدام SQLite مؤقتاً.")
                self.is_sqlite = True
    ...
    def _init_sqlite_tables(self, cursor, conn):
        ...
        if not cursor.fetchone():
            ...
            cursor.execute('INSERT INTO admins (user_id, username, added_at) VALUES (?, ?, ?)', (ADMIN_ID, 'المشرف الأساسي', now_epoch))
        conn.commit()
    ...
    def _init_postgres_tables(self, cursor, conn):
        ...
        if not cursor.fetchone():
            now_epoch = int(time.time())
            cursor.execute('INSERT INTO admins (user_id, username, added_at) VALUES (%s, %s, %s)', (ADMIN_ID, 'المشرف الأساسي', now_epoch))
        conn.commit()
        ...
        return self.execute_query(
            '''INSERT INTO scheduled_deletions (batch_id, delete_at, hours_duration, total_messages, created_at, status)
               ...
               ON CONFLICT (batch_id) DO UPDATE SET
                   ...
                   status = 'pending' ''',
            (batch_id, delete_at, hours_duration, total_messages, now_epoch)
        )
    ...
    def get_expired_scheduled_deletions(self):
        ...
        return self.execute_query(
            '''SELECT id, batch_id, hours_duration, total_messages FROM scheduled_deletions 
               WHERE status='pending' AND delete_at <= %s''',
            (now_epoch,),
            fetch=True
        ) or []
    ...
    def get_active_scheduled_deletion(self):
        ...
        rows = self.execute_query(
            '''SELECT id, batch_id, delete_at, hours_duration, total_messages, created_at 
               FROM scheduled_deletions WHERE status='pending' AND delete_at > %s 
    ...
    def add_channel_strike(self, channel_id: str, reason: str = "حذف منشور المجلد مبكراً") -> int:
        ...
        self.execute_query(
            ...
            (clean_ch, new_count, now_epoch, reason)
        )
        return new_count
        ...
        return self.execute_query(
            '''SELECT s.id, s.message_id, s.channel_id, s.batch_id, 
               ...
               WHERE (s.is_deleted_early IS NULL OR s.is_deleted_early = 0)
                 ...
                 AND (sd.delete_at IS NULL OR sd.delete_at > %s)
               ORDER BY s.id DESC''',
            (since_24h, min_del),
            fetch=True
        ) or []
    ...
    def update_last_milestone(self, request_id: int, milestone: int) -> bool:
        now_epoch = int(time.time())
        return self.execute_query(
            '''UPDATE listing_requests SET last_milestone = %s, updated_at = %s WHERE id = %s''',
            (milestone, now_epoch, request_id)
        )
    ...
    def add_to_blacklist(self, target_type: str, target_id: str, reason: str = None, blocked_by: str = None) -> bool:
        ...
        now_epoch = int(time.time())
        return self.execute_query(
            '''INSERT INTO blacklist (target_type, target_id, reason, blocked_by, created_at)
               VALUES (%s, %s, %s, %s, %s)
               ON CONFLICT (target_id) DO UPDATE SET
                   reason = EXCLUDED.reason,
                   blocked_by = EXCLUDED.blocked_by,
                   created_at = EXCLUDED.created_at''',
            (target_type, target_clean, reason, blocked_by, now_epoch)
        )
    ...
    def is_admin(self, user_id: int = None, username: str = None) -> bool:
        ...
        try:
            ...
            if user_id:
                cursor.execute('SELECT 1 FROM admins WHERE user_id = %s' if not self.is_sqlite else 'SELECT 1 FROM admins WHERE user_id = ?', (user_id,))
                if cursor.fetchone():
            ...
            if username:
                ...
                if username_clean:
                    ...
                    for variant in username_variants:
                        q_find = 'SELECT 1 FROM admins WHERE LOWER(TRIM(username)) = LOWER(TRIM(%s))' if not self.is_sqlite else 'SELECT 1 FROM admins WHERE LOWER(TRIM(username)) = LOWER(TRIM(?))'
                        cursor.execute(q_find, (variant,))
                        if cursor.fetchone():
                            logger.info(f"✅ تم العثور على المشرف باليوزرنيم: {variant}")
                            return True
                    
                    q_find2 = "SELECT 1 FROM admins WHERE LOWER(TRIM(REPLACE(username, '@', ''))) = LOWER(%s)" if not self.is_sqlite else "SELECT 1 FROM admins WHERE LOWER(TRIM(REPLACE(username, '@', ''))) = LOWER(?)"
                    cursor.execute(q_find2, (username_clean,))
                    if cursor.fetchone():
    ...
    def save_bot_user(self, user_id: int, username: str = None, first_name: str = None) -> bool:
        now_epoch = int(time.time())
        return self.execute_query(
            '''INSERT INTO bot_users (user_id, username, first_name, started_at)
               VALUES (%s, %s, %s, %s)
               ON CONFLICT (user_id) DO UPDATE SET
                   username=EXCLUDED.username,
                   first_name=EXCLUDED.first_name''',
            (user_id, username, first_name, now_epoch)
        )
    ...
    def save_listing_request(
        ...
        return self.execute_returning(
            '''INSERT INTO listing_requests
               (user_id, username, first_name, channel_id, channel_title, members_count, status, created_at, updated_at)
               VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
               RETURNING id''',
            (user_id, username, first_name, channel_id, channel_title, members_count, status, now_epoch, now_epoch)
        )
    ...
    def update_listing_status(self, request_id: int, status: str, user_id: int = None, reject_reason: str = None) -> bool:
        ...
        if reject_reason is not None:
            return self.execute_query(
                '''UPDATE listing_requests SET status = %s, reject_reason = %s, updated_at = %s
                   WHERE id = %s''',
                (status, reject_reason, now_epoch, request_id)
            )
        if user_id is None:
            return self.execute_query(
                '''UPDATE listing_requests SET status = %s, updated_at = %s
                   WHERE id = %s''',
                (status, now_epoch, request_id)
            )
        return self.execute_query(
            '''UPDATE listing_requests SET status = %s, updated_at = %s
               WHERE id = %s AND user_id = %s''',
            (status, now_epoch, request_id, user_id)
        )
    ...
    def save_sent_message(self, message_id: int, channel_id: str, chat_id: int, message_type: str, content: str, batch_id: int = None) -> bool:
        now_epoch = int(time.time())
        return self.execute_query(
            '''INSERT INTO sent_messages (message_id, channel_id, chat_id, message_type, content, batch_id, timestamp)
            VALUES (%s, %s, %s, %s, %s, %s, %s)''',
            (message_id, channel_id, chat_id, message_type, content, batch_id, now_epoch)
        )
    ...
    def save_pending_message(self, user_id: int, message_id: int, chat_id: int, message_type: str, content: str) -> int:
        ...
        now_epoch = int(time.time())
        return self.execute_returning(
            '''INSERT INTO pending_messages (user_id, message_id, chat_id, message_type, content, batch_id, created_at)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
            RETURNING id''',
            (user_id, message_id, chat_id, message_type, content, batch_id, now_epoch)
        )
    ...
    def add_admin(self, user_id: int = None, username: str = None) -> bool:
        ...
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
            q_ins = 'INSERT INTO admins (user_id, username, added_at) VALUES (%s, %s, %s)' if not self.is_sqlite else 'INSERT INTO admins (user_id, username, added_at) VALUES (?, ?, ?)'
            cursor.execute(q_ins, (user_id, username, now_epoch))
            conn.commit()
            ...
            if user_id and username:
                try:
                    ...
                    q_up1 = 'UPDATE admins SET username = %s WHERE user_id = %s AND username IS NULL' if not self.is_sqlite else 'UPDATE admins SET username = ? WHERE user_id = ? AND username IS NULL'
                    cursor2.execute(q_up1, (username, user_id))
                    q_up2 = 'UPDATE admins SET user_id = %s WHERE username = %s AND user_id IS NULL' if not self.is_sqlite else 'UPDATE admins SET user_id = ? WHERE username = ? AND user_id IS NULL'
                    cursor2.execute(q_up2, (user_id, username))
                    conn2.commit()
                    conn2.close()
                    return True
...
def handle_user_callback(call):
    ...
    if data.startswith("u_recheck_"):
        ...
        now_epoch = int(time.time())
        db.execute_query(
            '''UPDATE listing_requests SET channel_id=%s, channel_title=%s, members_count=%s,
               updated_at=%s WHERE id=%s AND user_id=%s''',
            (info["username"], info["title"], info["members_count"], now_epoch, request_id, user_id)
        )
        if not info["bot_ok"]:
            bot.send_message(
                chat_id,
        ...
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
            ...
            if actor_id:
                recent_req = db.execute_query(
                    "SELECT id, status, created_at FROM listing_requests WHERE user_id=%s AND (channel_id=%s OR channel_id=%s) ORDER BY id DESC LIMIT 1",
